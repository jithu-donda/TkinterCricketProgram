...
class IndStats():
	...
	def modifyBowlStats(self,txt):
		...
		for i in range(4):
			...
			txt[i].insert(INSERT,stats[i])
			# txt[i].config(text = stats[i])
	def writeBatStats(self,stats,player,i):
		...
		# print('playerVal',end = " ")
		# print(player)
		stats['BatArchives'][lenVal][player*3] = self.runs
		stats['BatArchives'][lenVal][player*3 + 1] = self.balls
		stats['BatArchives'][lenVal][player*3 + 2] = self.strikeRate
		return stats
	...
	def getEconomy(self,overs,runs):
		...
		if balls != 0:
			economy = format((float(runs*6.0)/balls),fL)
			return str()
		else :
			return 0
	
	def writeBowlStats(self,stats,player):
		...
		# print(player)
		player -= 1
		stats['BowlArchives'][lenVal][player*4] = str(int((self.balls)/6)) + "." + str((self.balls)%6)
		stats['BowlArchives'][lenVal][player*4 + 1] = self.runs
		stats['BowlArchives'][lenVal][player*4 + 2] = self.wickets
		stats['BowlArchives'][lenVal][player*4 + 3] = self.economy
		return stats
...
class Innings():
	...
	def writeStats(self,data,namesDict,teams):
		...
		for i in range(4):
			data = self.batStats[int(i/2)][i%2].writeBatStats(data,namesDict[teams[0][i%2]],i)
		for i in range(2):
			data = self.bowlStats[i].writeBowlStats(data,namesDict[teams[1][i]])
		for j in range(4):
			i = j+1
	...
	def addWide(self):
		self.score += 1
		self.listOfEntries.append("extra")
		currBalls = self.balls
		overs = int(currBalls/6)
		self.currBowler = (overs % 2)
	...
	def incorrectEntry(self):
		...
		if (lastEntry == "wicket"):
			...
			self.batStats[int((self.wickets)/2)][self.currBatsman].redoScore(int(lastEntry))
			self.bowlStats[self.currBowler].redoScore(int(lastEntry))
		# print(self.listOfEntries)
	...
	def addWicket(self):
		...
		self.wickets += 1
		self.listOfEntries.append("wicket")
		self.updateCurrBatsmanAndBowler()
		if (self.wickets == 4):
			return "Innings Over"
		# if (self.currInn == 2):
	...
	def addScore(self,val):
		...
		self.listOfEntries.append(str(val))
...
class Match():
	...
	def clickedStartMatch(self):
		...
		for inn in range(self.noOfInnings):
			for i in range(4):
				self.teams[inn][(i+inn)%2].append(self.playerNames[i].get())
		...
		self.runsButtons(17,self.window1,self.noOfInnings)
		self.window1.mainloop()
	
	def runsButtons(self,val,window,noOfInnings):
		# self.btns = []
		# for i in [0,1,2,4]:
		self.bg_colour = 'aquamarine'
		self.fg_colour = 'firebrick4'
		...
		runsbgColour = ['ivory2','yellow','yellow','green2']
		...
		runsWidthVal = 5
		runsHeightVal = 3
		bottomTexts = ["Wicket","Incorrect Entry","Wide","NoBall"]
		bottombg = ['red','red','DarkOrange1','DarkOrange1']
		bottomfg = ['black','black','brown4','brown4']
		...
		for i in range(4):
			btn = Button(window, text=runsTexts[i],bg = runsbgColour[i],fg = runsfgColour[i],command=runsCommands[i],width = runsWidthVal,height = runsHeightVal)
			btn.grid(column = i+1,row = val,pady = 10)
		
		for i in range(4):	
			btn = Button(window, text=bottomTexts[i], bg = bottombg[i],fg = bottomfg[i],command=bottomCommands[i],height = bottomHeight)
		...
		for i in range(1):
			...
			lbl = Label(window, textvariable = self.currInnStats[i], bg = self.bg_colour, fg = 'red', font = 'Arial 120 bold')
		...
		for inn in range(noOfInnings):
			...
			for i in range(4):
				...
				combo['values']= (self.teams[inn][0][0],self.teams[inn][0][1])
			...
			for i in range(2):
				combo = ttk.Combobox(window,width = 10)
				combo['values']= (self.teams[inn][1][0],self.teams[inn][1][1])
				combo.current(i%2) #set the selected item
		...
		colVals = [5,5,10,10,5]
		rowVals = [val*2+2,val*(self.noOfInnings + 3),val*(self.noOfInnings + 3),val*(self.noOfInnings + 3),val*(self.noOfInnings + 3)+1]
		commands = [self.setOrder,self.endMatch,self.endDay,self.copyData,self.quitWindow]
		padyVals = [3,30,30,30,0]
		bgs = ['yellow','DarkOrange1','DarkOrange1']
		fgs = ['red','white','white']
	...
	def setOrder(self):
		for inn in range(2):
			if self.batsmanNames[inn][0].get() == self.teams[inn][0][1]:
				self.teams[inn][0].reverse()
				for i in range(4):
					self.batsmanNames[inn][i].current((i+1)%2) #set the selected item
			else:
				for i in range(4):
					self.batsmanNames[inn][i].current(i%2) #set the selected item
			if self.bowlerNames[inn][0].get() == self.teams[inn][1][1]:
				self.teams[inn][1].reverse()
				for i in range(2):
	...
	def endMatch(self):
		...
		if (result):
			...
			for i in range(self.noOfInnings):
				data = self.innings[i].writeStats(data,namesDict,self.teams[i])
			# print(stats)
	...
	def endDay(self):
		self.endMatch()
		os.system("libreoffice " + self.currFile)
...
match = Match("cricStats.ods","oldCricStats.ods")	